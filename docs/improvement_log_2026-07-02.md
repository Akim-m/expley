...
2. **EPSS version staleness** — all claims are vs v3/v4 history; claims must name the version-era (v3/v4; v4 live 2025-03-17 per first.org) — an earlier 'v5 shipped 2026-06-15' survey claim did not verify (RE audit 2026-07-03).
...
| `effort_metrics.py` (stratified pooled-bootstrap PR-AUC CIs, coverage paired deltas) + `keep_scores` flag + `scripts/inwild_remetric.py` → `artifacts/inwild_remetric.json`, `docs/figures/fig_coverage_effort.png` | A1 core: restate the headline in the field's idiom; the "PR-AUC tied" claim needed its noise band | **HEADLINE CHANGED: the tie dissolves in structural's favor.** Pooled PR-AUC@30 structural 0.0071 [0.0058,0.0095] vs EPSS 0.0040 [0.0032,0.0054] — bands don't overlap (same @90: 0.0134 vs 0.0078). Coverage crossover: EPSS holds top-1% (Δ−0.003 n.s.), structural wins from 5–10% effort (Δ+0.200 [0.104,0.296] @30, +0.238 win 15/15 @90). Sanity gate: AUC@30 delta +0.0998 vs parity's +0.1001 (see RE round below for the honest root cause). EPSS-version stamped (v3/v4-era). 3 m 17 s / 1.33 GB |
| A1 RE round (1 verifier × 3 loops) — headline STRENGTHENED + 3 fixes | Deliverable rule | (1) The proper **paired pooled bootstrap** (identical rows, AP difference): **Δ@30 +0.0031 [+0.0013,+0.0054], Δ@90 +0.0057 [+0.0023,+0.0091], 1000/1000 replicates positive** — now emitted by the script and cited as THE significance statement (per-arm band non-overlap was the weaker reading). (2) Subcohort boundary fixed (`>=` not `>`; 332 rows @30, ≤1e-5 AP effect). (3) Wording corrected: crossover between 1–2% effort, significant from 5% (90d)/10% (30d); at ≤1% effort the arms are statistically indistinguishable (point estimates favor EPSS at 0.5%, n.s.). (4) **EPSS "v5 shipped 2026-06-15" survey claim did NOT verify** against FIRST (latest published model: v4, live 2025-03-17; the 2026-06-15 item is a press release) — corrected in every doc it propagated to. (5) The +0.0998-vs-+0.1001 shift is NOT a data refresh: the EPSS arm reproduces parity bit-exactly; the structural arm's xgb fits changed deterministically under the refactored row ordering (hist training is row-order-sensitive) — structural recall@top-1%@30 moved 0.051→0.065. Parity re-baseline at HEAD queued |
...
| L1 Vulnrichment SSVC miner (`fetch/vulnrichment.py`): streamed git-log parser for Exploitation transitions, earliest-wins, label-only doctrine in the module docstring | The one statistics-approved PR-AUC lever: timestamped `active` assessments with backfill since mid-2024 | Parser + frame contract test-pinned (split-line values, deletes, non-CVE files, ns-UTC dtype). Clone: first attempt died mid-transfer (early EOF, ~213 MB in); retry with git keepalive settings (`http.lowSpeedLimit/lowSpeedTime`) landed the full repo — **10,881 commits**. Full-history mine + label-value measurement delegated to an Opus subagent (main session verifies+commits — token-economy directive 2026-07-03). **MEASURED REDUNDANT — not wired.** Mine: 77 s / 471 MB, 39,114 CVEs (1,650 active, 37,567 poc; 2024-05-08→2026-07-01). Vs KEV∪VulnCheck (4,969): **11 net-new CVEs, 6 usable events**; overlap dates lag existing sources by median **+989 d** (0.18% earlier) — SSVC assessments follow the catalogs, no timing gain. Subagent's parser validation also caught 45 capitalized `"Active"` typos the case-sensitive regex dropped (fixed + test-pinned; suite 388 passed). The last untested free historical source now confirms the ~396/1,310-event data ceiling. `artifacts/l1_vulnrichment_measure.json` |
| A3 seed variance + bounded search (`scripts/a3_seed_and_tune.py`, `artifacts/a3_seed_and_tune.json`) — **tuned config NOT adopted** | The +0.100 headline had no seed-variance estimate; hyperparameters were untuned defaults | **Seed variance is exactly ZERO** (sd 0.0 over seeds 0–4: no stochastic components in the config — deterministic hist training), so the headline has no seed fragility by construction. Search (16 configs tuned on origins[:8]): winner lr 0.03/depth 4/σ 2.0 (tune AUC@30 0.822) but confirm-set deltas cross zero (+0.012 @30 [−0.010,+0.033], +0.016 @90 [−0.005,+0.037], win 0.71/0.86) → default stays per the significance gate; shallower-smoother config recorded as a candidate when more origins accumulate. 14 m 30 s / 1.36 GB |
...
| What was done | Why |
|---|---|
| Methods survey (2024–26) reported; findings folded into the design | Reframes the headline: EPSS-version claim corrected by RE audit 2026-07-03: FIRST's latest published model is v4 (live 2025-03-17); a 'v5 shipped 2026-06-15' claim from the survey did NOT verify (that date is a FIRST press release). Claims must state the version-era tested (v3/v4); the PR-AUC "tie" is inside the noise band at 1310 positives (Boyd 2013); nobody published time-to-in-wild survival modeling 2024–26 — the framing is the differentiator. |
