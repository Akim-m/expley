...
- **Cure model — calibration/discrimination trade-off** — log-logistic latency is AIC-selected; held-out isotonic recalibration is the only one of three approaches (vs in-sample, cross-fit) that improved test IPA, but it costs ~0.02 c-index, so it's opt-in. Open: a recalibration that recovers full discrimination (the cross-fit fold-vs-full mismatch is the obstacle), and per-cause cure fractions for the competing-risks setting.
