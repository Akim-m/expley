...
- [x] **Whole-pipeline speed/memory bundle** (2026-07-03, branch `speed-memory-bundle` — spec `docs/superpowers/specs/2026-07-03-pipeline-improvement-design.md`, running record **[`docs/improvement_log_2026-07-02.md`](docs/improvement_log_2026-07-02.md)** with every change + reason + before/after) — measured `/usr/bin/time -v` baselines for all four pipeline paths, then: **cached landmark-EPSS loader** (`load_epss_at_landmark(s)`; scripts stop re-streaming the 375M-row file — **0.57 s vs ~4 min, ~450×**; guards: snapshot + trajectory-columns + corpus coverage + per-CVE `published` exact match), **backtest hoist** (test frame prepared once; **38.5→35.15 s, −8.7%**, report bit-identical), **vectorized CVSS parse + CWE explode/crosstab** (full-corpus parquets frame-identical), **thread-parallel hill-climb** (`n_workers`, serial-identical by test), **XGB `validation="random"` early stopping** — measured A/B on 15 origins: 25% faster but **AUC −0.043 (CI excl. 0) → adoption rejected**, ships opt-in (honest negative; feeds the hyperparameter workstream). Closed with a 2-agent × 5-loop adversarial RE audit that found and fixed **6 real breaks** (CVSS empty-value/bytes parse drift, CWE None-in-list crash + dup-cve_id contamination, cache published-drift wrong-data risk, lone-event validation steal) + observability gaps (`n_skipped_origins`, CPU-fallback warning). Suite 373 passed; peak RSS ≤1.46 GB (24% of the 6 GB gate) throughout. **A1–A3 + L1 executed same day:** **A1 re-metric supersedes the "PR-AUC tied" verdict — structural WINS PR-AUC significantly** (paired pooled bootstrap, identical rows: Δ@30 +0.0031 [+0.0014,+0.0056], Δ@90 +0.0057 [+0.0020,+0.0088], 99.8% of replicates positive; coverage/effort curves in `docs/figures/fig_coverage_effort.png` — arms indistinguishable at ≤1% effort, structural +0.24 coverage at 10% effort, win 15/15 @90d). A2 LambdaRank **rejected** (AUC@90 −0.164, win 0/15). A3: **zero seed variance** (deterministic config); bounded search n.s. on confirm origins → defaults retained. L1 Vulnrichment SSVC **measured redundant, not wired** (11 net-new / 6 usable events vs KEV∪VulnCheck, dates lag median +989 d) — the last untested free historical label source, confirming the data ceiling. An "EPSS v5" survey claim did not verify (FIRST's latest model is v4, live 2025-03-17) and was retracted. Full detail: the improvement log + `docs/performance_report_2026-07-03.pdf`.
